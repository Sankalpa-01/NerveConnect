...
const { generatePrescription, storeAnalysis } = require('./controllers/aiAnalysis')
...
if (process.env.ENABLE_DB_STATUS === 'true' || process.env.NODE_ENV !== 'production') {
  ...
  app.get('/internal/db-status', async (req, res) => {
    try {
      ...
      return res.status(500).json({ error: 'Failed to fetch DB status', details: err?.message || err })
    }
  })
}
