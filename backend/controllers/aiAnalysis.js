...
async function generatePrescription(req, res) {
  try {
    ...
    const inputs = templateString
      ...
      .replace('{heartRate}', appointment.heartRate?.toString() || 'Not provided')
      .replace('{temperature}', appointment.temperature?.toString() || 'Not provided')
      .replace('{oxygenSaturation}', appointment.oxygenSaturation?.toString() || 'Not provided')
    ...
    if (process.env.USE_MOCK_GEMINI === 'true') {
      ...
      const prescription = `I recommend Paracetamol 500mg, 1 tablet every 6 hours as needed for pain and fever for 5 days. Continue hydration and rest. Follow up in 3-5 days or sooner if symptoms worsen. (Auto-generated mock)`
      // Save mock analysis to DB
      try {
        const db = await getDb()
        const col = db.collection('ai_analysis')
        await col.insertOne({ appointment, prescription, createdAt: new Date(), mock: true })
      } catch (e) {
        console.warn('Failed to save mock AI analysis to MongoDB:', e?.message || e)
      }
    ...
    try {
      ...
      console.error('Gemini API request failed:', err?.response?.status, err?.message)
      const allowMockFallback = process.env.USE_MOCK_GEMINI === 'true' || process.env.FALLBACK_TO_MOCK_ON_GEMINI_FAIL === 'true' || process.env.NODE_ENV !== 'production'
      if (allowMockFallback) {
        console.warn('Falling back to mock prescription due to Gemini failure')
        const symptoms = appointment.symptoms || 'Not provided'
        const diagnosis = appointment.diagnosis || 'Not provided'
        const prescription = `I recommend Paracetamol 500mg, 1 tablet every 6 hours as needed for pain and fever for 5 days. Continue hydration and rest. Follow up in 3-5 days or sooner if symptoms worsen. (Auto-generated mock due to Gemini failure)`
        // Save mock analysis to DB
        try {
          const db = await getDb()
          const col = db.collection('ai_analysis')
          await col.insertOne({ appointment, prescription, createdAt: new Date(), mock: true, fallback: true })
        } catch (e) {
          console.warn('Failed to save mock AI analysis to MongoDB:', e?.message || e)
        }
        return res.json({ prescription })
      ...
      return res.status(502).json({ error: 'Failed to call Gemini API', details: err?.response?.data || err?.message })
    }

    const textResponse = gres.data?.candidates?.[0]?.content?.parts?.[0]?.text

    const prescription = textResponse || 'Prescription generation failed or returned empty.'
...
async function storeAnalysis(req, res) {
  try {
    ...
    if (!appointmentId || !aiAnalysis) {
      return res.status(400).json({ error: 'Missing appointmentId or aiAnalysis' })
    }
    ...
    return res.json({ message: 'AI analysis stored', appointment: result.value })
  } catch (err) {
