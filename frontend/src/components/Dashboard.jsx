...
const EnhancedDoctorDashboard = () => {
  ...
  const currentPrescription = selectedAppointment
    ? prescriptionData[selectedAppointment._id] // Use _id
    : "";
  ...
  const submitPatient = async (e) => {
    ...
    try {
      ...
      const data = await res.json(); // data is the new patient object
      if (!res.ok) throw new Error(data.error || "Failed to submit patient");
      ...
      const notification = {
        ...
        message: `New patient ${data.name} added`,
        type: "success",
      ...
      console.error("submitPatient error:", err);
  ...
  const submitAppointment = async (e) => {
    ...
    try {
      ...
      const data = await res.json(); // data is the new appointment object
      if (!res.ok)
  ...
  const aianalysis = async (appointment) => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/ai_analysis`, {
        method: "POST",
        credentials: "include",
      ...
      setPrescriptionData((prev) => ({
        ...prev,
        [appointment._id]: data.prescription, // Use _id and data.prescription
      }));
  ...
  const handleAISave = async () => {
    try {
      await fetch(`${API_BASE_URL}/api/appointments/ai_analysis`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          appointmentId: selectedAppointment?._id, // Use _id
      ...
      if (selectedAppointment) {
        setPrescriptionData((prev) => ({
          ...prev,
          [selectedAppointment._id]: "", // Use _id
        }));
      }
    } catch (error) {
  ...
  const handlePrescriptionUpdate = () => {
    if (selectedAppointment) {
      setPrescriptionData((prev) => ({
        ...prev,
        [selectedAppointment._id]: tempPrescription, // Use _id
      }));
    }
    setIsEditingPrescription(false);
  };
    ...
    >
      ...
      {showAddAppointmentModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          ...
          >
            <div className="flex justify-between items-center p-8 border-b border-gray-100 bg-gradient-to-r from-green-50 to-teal-50 rounded-t-3xl shrink-0">
              <h2
                className={`text-2xl font-bold ${
                  darkMode ? "text-white" : "text-gray-900"
                }`}
